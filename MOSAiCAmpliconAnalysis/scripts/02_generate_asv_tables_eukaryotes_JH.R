...
# Input: Fastqs
# Main output: taxonomy_eukaryotes.csv, asv_table_eukaryotes.csv
...
#Define all relevant analysed runs (each directory contains forward and reverse 18S fastq reads)
...
####Filter and trim and dereplicate####
...
####Taxa assignmennt####
taxa <- assignTaxonomy(seqtab.nochim, "pr2_version_5.0.0_SSU_dada2.fasta.gz",multithread = T,
...
names(seq)=paste0("euk_asv",seq(length(seq)))
...
rownames(euk_taxa)=names(seq)[mi]
save(euk_taxa,file=paste0(temp_dir,"/taxa_Euk.Rdata"))
write.csv(euk_taxa,file="./output/taxonomy_eukaryotes.csv", row.names=TRUE)
...
write.csv(abundMatRaw,file="./output/asv_table_eukaryotes.csv", row.names=TRUE)
