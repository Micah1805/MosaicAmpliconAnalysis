...
# Input: environmental_metadata.csv, Light.csv
# Main output: environmental_data_depths.csv, environmental_data_mean.csv", 
#              environmental_data_light_2.csv, environmental_data_light_RDA.csv
...
#Modify environmental data and merge it with light data (by ID)
Env_tot$Date <- as.Date(Env_tot$Date)
Env_tot <- tibble::rownames_to_column(Env_tot, "ID")
colnames(Env_tot)[6] <- "Depth"
Env_tot_Light <- full_join(Env_tot,Light,by=c("ID"))

#Remove all unneeded columns and rows
Env_tot_Light <- Env_tot_Light[1:21]
Env_tot_Light <- Env_tot_Light[1:733,]
Env_tot_Light <- Env_tot_Light[order(Env_tot_Light$Depth),]
Env_tot_Light <- Env_tot_Light %>% relocate("Date.y")
Env_tot_Light <- Env_tot_Light[-1]

#Remove duplicates by averaging values
Env_tot_Light_m_1 <- Env_tot_Light %>% group_by(ID) %>% dplyr::summarise(across(c(1:19),mean)) %>% distinct 
