################################################################################
########################Normalize abundance eukaryotes##########################
################################################################################
...
#####Prapare tables for microeco####

#Load data
#Metadata
Meta_Euk <- read_excel("~/AWI/MOSAiC/Sequencing_Prok_Euk/Metadata/Water_only/Metadata_Mosaic_CTD_18S.xlsx")

#Raw abundances
load("~/AWI/MOSAiC/Sequencing_Prok_Euk/Output_Euk/Water_only/18S_water/RawAbundanceMat_Euk.Rdata")
...
Euk<-as.data.frame(t(abundMatRaw))
...
Water_sort_Euk$Date_Category<-paste(Water_sort_Euk$"collection date",Water_sort_Euk$Category)
...
Euk_water_m <- Water_sort_Euk %>% group_by(Date_Category,Category) %>% summarise(across(c(8:6136),mean)) %>% distinct 
...
save(normEukAbund,file=paste0(path,"/normEukAbund.Rdata"))